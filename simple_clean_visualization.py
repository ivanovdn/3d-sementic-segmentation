...
def create_clean_visualization(
    labeled_points, hide_clutter=True, emphasis_mode="furniture"
):
    """
...
def visualize_clean_scene(labeled_points, save_path=None, emphasis_mode="furniture"):
    ...
    pcd, colors = create_clean_visualization(
        labeled_points, hide_clutter=True, emphasis_mode=emphasis_mode
    )
...
def create_clean_legend(labeled_points, emphasis_mode="furniture", save_path=None):
    ...
    # Get colors used
    _, colors = create_clean_visualization(
        labeled_points, hide_clutter=True, emphasis_mode=emphasis_mode
    )
    ...
    ax.set_xticks([])
    ax.set_title(
        f"Clean Semantic Segmentation - {emphasis_mode.title()} Mode (Clutter Removed)",
        fontsize=16,
        fontweight="bold",
        pad=20,
    )
...
def create_comparison_view(labeled_points, save_path=None):
    ...
    pcd_original, _ = create_clean_visualization(
        labeled_points, hide_clutter=False, emphasis_mode="balanced"
    )

    # Clean without clutter
    pcd_clean, _ = create_clean_visualization(
        labeled_points, hide_clutter=True, emphasis_mode="furniture"
    )
    ...
    vis = o3d.visualization.Visualizer()
    vis.create_window(
        window_name="Comparison: Original vs Clean", width=1600, height=800
    )
    ...
    if save_path:
        ...
        o3d.io.write_point_cloud(
            save_path.replace(".ply", "_original.ply"), pcd_original
        )
...
def interactive_clean_visualization(labeled_points, output_dir="clean_visualization"):
    ...
    while True:
        ...
        if choice == "1":
            pcd = visualize_clean_scene(
                labeled_points,
                save_path=output_path / "furniture_focus.ply",
                emphasis_mode="furniture",
            )
            create_clean_legend(
                labeled_points, "furniture", output_path / "legend_furniture.png"
            )
            ...
            create_clean_legend(
                labeled_points, "structural", output_path / "legend_structural.png"
            )
            ...
            create_clean_legend(
                labeled_points, "balanced", output_path / "legend_balanced.png"
            )

        elif choice == "4":
            create_comparison_view(
                labeled_points, save_path=output_path / "comparison.ply"
            )
            ...
            for mode in ["furniture", "structural", "balanced"]:
                ...
                create_clean_legend(
                    labeled_points, mode, output_path / f"legend_{mode}.png"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mode",
        type=str,
        choices=["furniture", "structural", "balanced", "interactive"],
        default="interactive",
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="clean_visualization",
        help="Output directory for saved files",
    )
    parser.add_argument(
        "--save_only", action="store_true", help="Save files without opening viewer"
    )
    ...
    try:
        ...
        if labeled_points.shape[1] != 7:
            raise ValueError(
                f"Expected 7 columns [x,y,z,r,g,b,label], got {labeled_points.shape[1]}"
            )
    ...
    if args.mode == "interactive":
        ...
        pcd, _ = create_clean_visualization(
            labeled_points, hide_clutter=True, emphasis_mode=args.mode
        )
        ...
        create_clean_legend(
            labeled_points, args.mode, output_path / f"legend_{args.mode}.png"
        )
